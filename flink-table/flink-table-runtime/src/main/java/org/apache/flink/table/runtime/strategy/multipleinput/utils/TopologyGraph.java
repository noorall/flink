...
/**
 * A data structure storing the topological and input priority information of an {@link WrappedNode}
 * graph.
 */
@Internal
public class TopologyGraph {

    private final Map<WrappedNode<?>, TopologyNode> nodes;

    TopologyGraph(List<WrappedNode<?>> roots) {
        this(roots, Collections.emptySet());
    }

    TopologyGraph(List<WrappedNode<?>> roots, Set<Integer> boundaries) {
        ...
        AbstractWrappedNodeExactlyOnceVisitor visitor =
                new AbstractWrappedNodeExactlyOnceVisitor() {
                    ...
                    protected void visitNode(WrappedNode<?> node) {
                        ...
                        for (WrappedEdge<?> inputEdge : node.getInputEdges()) {
     ...
     */
    boolean link(WrappedNode<?> from, WrappedNode<?> to) {
     ...
     * nothing.
     */
    void unlink(WrappedNode<?> from, WrappedNode<?> to) {
        TopologyNode fromNode = getOrCreateTopologyNode(from);
        TopologyNode toNode = getOrCreateTopologyNode(to);
     ...
     */
    Map<WrappedNode<?>, Integer> calculateMaximumDistance() {
        Map<WrappedNode<?>, Integer> result = new HashMap<>();
     ...
     * B to node A.
     */
    void makeAsFarAs(WrappedNode<?> a, WrappedNode<?> b) {
    ...
    boolean canReach(WrappedNode<?> from, WrappedNode<?> to) {
        TopologyNode fromNode = getOrCreateTopologyNode(from);
        TopologyNode toNode = getOrCreateTopologyNode(to);
    ...
    private TopologyNode getOrCreateTopologyNode(WrappedNode<?> node) {
        // NOTE: We treat different `BatchExecBoundedStreamScan`s with same `DataStream` object as
        // the same
        for (Map.Entry<WrappedNode<?>, TopologyNode> entry : nodes.entrySet()) {
            WrappedNode<?> key = entry.getKey();
            if (node.isSameNode(key)) {
                return entry.getValue();
            }
        }
        return nodes.put(node, new TopologyNode(node));
    }

    /** A node in the {@link TopologyGraph}. */
    private static class TopologyNode {
        private final WrappedNode<?> execNode;
        private final Set<TopologyNode> inputs;
        private final Set<TopologyNode> outputs;

        private TopologyNode(WrappedNode<?> execNode) {
