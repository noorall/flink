...
public class BlockingInputInfo implements BlockingResultInfo {
    ...
    /**
     * Used in VertexParallelismAndInputInfosDecider, which means that there are relationships
     * between multiple inputs, if the data corresponding to a specific join key from one input is
     * split, the corresponding join key data from the other inputs must be duplicated (meaning that
     * it must be sent to the downstream nodes where the split data is sent).
     */
    private final boolean interInputsKeyCorrelation;

    /**
     * Used in VertexParallelismAndInputInfosDecider,which means that the data corresponding to a
     * specific join key must be sent to the same downstream subtask.
     */
