...
/**
 * A data structure storing the topological and input priority information of an {@link
 * StreamExecNode} graph.
 */
@Internal
class TopologyGraph {

    private final Map<StreamExecNode, TopologyNode> nodes;

    TopologyGraph(List<StreamExecNode> roots) {
        this(roots, Collections.emptySet());
    }

    TopologyGraph(List<StreamExecNode> roots, Set<StreamExecNode> boundaries) {
     ...
     */
    boolean link(StreamExecNode from, StreamExecNode to) {
     ...
     * nothing.
     */
    void unlink(StreamExecNode from, StreamExecNode to) {
        TopologyNode fromNode = getOrCreateTopologyNode(from);
        TopologyNode toNode = getOrCreateTopologyNode(to);
     ...
     */
    Map<StreamExecNode, Integer> calculateMaximumDistance() {
        Map<StreamExecNode, Integer> result = new HashMap<>();
     ...
     * B to node A.
     */
    void makeAsFarAs(StreamExecNode a, StreamExecNode b) {
    ...
    boolean canReach(StreamExecNode from, StreamExecNode to) {
        TopologyNode fromNode = getOrCreateTopologyNode(from);
        TopologyNode toNode = getOrCreateTopologyNode(to);
    ...
    private TopologyNode getOrCreateTopologyNode(StreamExecNode execNode) {
    ...
    private static class TopologyNode {
        private final StreamExecNode execNode;
        private final Set<TopologyNode> inputs;
        private final Set<TopologyNode> outputs;

        private TopologyNode(StreamExecNode execNode) {
